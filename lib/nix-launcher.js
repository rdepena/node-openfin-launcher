        ...
        try {
            assetUtilities.downloadRuntime(config.runtime.version, function(err, runtimePath) {
                if (err) {
                    ...
                    var arguments = assetUtilities.getRunningOs() !== assetUtilities.OS_TYPES.linux ? config.runtime.arguments : '--no-sandbox'
                    var of = spawn(runtimePath, ['--startup-url="' + combinedOpts.configPath + '" ', arguments], {
                        encoding: 'utf8'
                    });
